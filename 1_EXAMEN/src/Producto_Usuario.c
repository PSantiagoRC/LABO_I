...
int rel_Producto_Usu_alta(eProducto arrayProductos[], eUsuario arrayUsuarios[],
	...
	sellerIndex = eUsuario_findIndexByCorreo(arrayUsuarios, tamUsuarios, correoSeller);
	sellerId = arrayUsuarios[sellerIndex].PK_idUsuario;

	if (arrayProductos != NULL && arrayUsuarios != NULL) {
		if (tamProductos > 0 && tamUsuarios > 0) {
			if (emptyIndexProductos >= 0 && sellerIndex >= 0 && sellerId >= 0) {
				...
				if(retLoad == 0){
					...
					if (valid_siNo(
							...
							"Error. Ingrese S o N.\n", 3)) {
						if (addProducto(&arrayProductos[emptyIndexProductos], tamProductos,
								productoBuffer.nombreProducto, productoBuffer.precio,
								productoBuffer.categoria, productoBuffer.stock,
								productoBuffer.PK_idProducto) == 0) {
							arrayProductos[emptyIndexProductos].FK_idSeller = productoBuffer.FK_idSeller; //Se carga por fuera de addProducto
							...
							puts("Error en la carga de proudcto a base de datos.");
							retorno = -6;
						}
					} else {
				...
				puts("Base de productos completa o no se encuentra al vendedor");
